namespace MMEdit
{
    ...
    public static class LambdaMaker
    {
        ...
        public static Func<TObj, TProp> GetGetter<TObj, TProp>(PropertyInfo propertyInfo)
        {
            string propertyName = propertyInfo.Name;
            return (Func<TObj, TProp>)_getterCache.GetOrAdd(typeof(TObj), new ConcurrentDictionary<string, object>())
                .GetOrAdd(propertyName, GenerateGetterLambda<TObj, TProp>(propertyInfo));
        }
        ...
        private static Func<TObj, TProp> GenerateGetterLambda<TObj, TProp>(PropertyInfo propertyInfo)
        {
            string propertyName = propertyInfo.Name;
            if (propertyInfo.PropertyType != typeof(TProp))
            {
